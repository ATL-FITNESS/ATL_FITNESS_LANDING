    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOtp({
        ...
        options: {
          ...
          emailRedirectTo: `${window.location.origin}/auth/callback`, // Redirect to our callback route
        }
      });

      if (error) {
        if (error.message.includes('User not found') || error.message.includes('Invalid login credentials')) {
          throw new Error('No member account found with this email address. Please contact the gym administrator.');
        }
        throw error;
      }
